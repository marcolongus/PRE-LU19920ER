...
int main(void) {
	...
	ofstream velocity_data("data/velocity_data.txt", ios_base::app);
	...
	for (KIND vel_crit = 0.055; vel_crit < 0.15; vel_crit += 0.005) {
		...
		for (size_t n_simulaciones = 0; n_simulaciones < 300; n_simulaciones++) {
			...
			for (int p=0; p<N; p++) {
				...
				//velocity_data << system[p].velocity << endl;
			}
	 
	 		// Select fastest particle at infected state
			auto maxima_velocidad_iterator = max_element(velocities.begin(), velocities.end());
			int max_index = distance(velocities.begin(), maxima_velocidad_iterator);
			double max_velocity = *maxima_velocidad_iterator;	 
			cout << "Max Vel = " << max_velocity << " at index " << max_index << endl;
			//system[max_index].set_infected();
			//state_vector[system[max_index].get_state()]++;
			//state_vector[0]--;
			print_state(state_vector);
